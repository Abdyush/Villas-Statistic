    ...
    def first_page():
        ...
        def main_work(e, last_year, this_year):
            ...
            for sheets in list_sheet:
                for sheet in sheets:
                    perfect_new_month_statistic(sheets=sheets, sheet=sheet, sheet_index=sheets.index(sheet), sheets_index=list_sheet.index(sheets))
        ...
        row_filepicker_1.controls.append(ft.ElevatedButton(text="Выберите файл с Occupancy за прошлый год", on_click=select_first_file))
        
        filepicker_2 = ft.FilePicker(on_result=return_second_file)
        row_filepicker_2.controls.append(ft.ElevatedButton(text="Выберите файл с Occupancy за текущий год", on_click=select_second_file))
        ...
        btn = ft.FilledButton(text='Запуск', width=300, height=45, on_click=start_programm)
        page.add(btn)
